...
if ( ! function_exists('_form_common'))
{
	...
	function _form_common($type='text', $data='', $value='', $label='', $extra='', $tooltip = '')
	{
		...
		//try to set form value in case of validation
		$defaults['value'] = set_value($defaults['name']);
		...
		if (function_exists('form_error'))
		{
			...
			{
				$tooltip = '<span class="help-block">'.$tooltip.'</span>';
			}
		}
...
<div class="form-group {$error}">
	<label class="control-label col-lg-3 col-md-3 col-sm-4" for="{$defaults['name']}">{$label}</label>
	<div class="controls col-lg-6 col-sm-8 col-md-6">
		 <input {$output} {$extra} />
		{$tooltip}
	</div>
</div>
...
/**
 * Generates checkbox input with labels
 **/

if(! function_exists('form_checkbox_label')) {
